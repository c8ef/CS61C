...
#pragma omp parallel
  {
    ...
    for (int i = num; i < ARRAY_SIZE; i += allThreads)
      z[i] = x[i] + y[i];
  }
}
...
#pragma omp parallel
  {
    int allThreads = omp_get_num_threads();
    int num = omp_get_thread_num();
    int CHUNK = ARRAY_SIZE / allThreads;

    for (int i = num * CHUNK;
         i < (num == allThreads - 1 ? ARRAY_SIZE : (num + 1) * CHUNK); ++i)
      z[i] = x[i] + y[i];
  }
}
...
#pragma omp parallel
  {
    int allThreads = omp_get_num_threads();
    int num = omp_get_thread_num();
    int CHUNK = arr_size / allThreads;
    double inner = 0;

    for (int i = num * CHUNK;
         i < (num == allThreads - 1 ? arr_size : (num + 1) * CHUNK); ++i)
      inner += x[i] * y[i];
